...
test("ingestion activities invoke shell wrappers through bash from repo scripts dir", async () => {
  const calls = [];
  ...
  await activities.buildGlobalModel(["--country", "DE"]);
  ...
  assert.deepEqual(calls[0].args.slice(1), ["--country", "DE"]);
  assert.deepEqual(calls[1].args.slice(1), ["--country", "DE"]);
