...
function defaultStateDir(cwd) {
  const normalizedCwd = path.resolve(cwd);
  if (normalizedCwd.endsWith(path.join("services", "orchestrator"))) {
    return path.join(normalizedCwd, "state");
  }
...
function loadConfig() {
  ...
  const stateDir = resolvePath(process.env.STATE_DIR, defaultStateDir(cwd));
