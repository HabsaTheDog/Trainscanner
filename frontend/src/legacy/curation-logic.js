...
export function initCurationApp() {
  ...
  let selectedDraftMergeIds = new Set();
  ...
  function inferCandidateCategory(candidate) {
    ...
    if (text.includes('subway') || text.includes('u-bahn') || text.includes('ubahn') || text.includes('metro')) {
      return 'subway';
    }
    if (text.includes('north') || text.includes('south') || text.includes('east') || text.includes('west') || text.includes('secondary')) {
      return 'secondary';
    }
    if (text.includes('main') || text.includes('hbf') || text.includes('hauptbahnhof') || text.includes('rail') || text.includes('platform')) {
      return 'main';
    }
    return 'other';
  }
  ...
  function getCandidateByStationId(stationId) {
    ...
    return activeClusterDetail.candidates.find((candidate) => candidate.canonical_station_id === stationId) || null;
  }
  ...
  function compareCandidateRank(a, b) {
    ...
    const safeRankA = Number.isFinite(rankA) && rankA > 0 ? rankA : Number.MAX_SAFE_INTEGER;
    const safeRankB = Number.isFinite(rankB) && rankB > 0 ? rankB : Number.MAX_SAFE_INTEGER;
    if (safeRankA !== safeRankB) {
      return safeRankA - safeRankB;
    }
    return String(a?.canonical_station_id || '').localeCompare(String(b?.canonical_station_id || ''));
  }

  function sortCandidatesByRank(candidates) {
    return (Array.isArray(candidates) ? candidates.slice() : []).sort(compareCandidateRank);
  }

  function sortStationIdsByCandidateRank(stationIds) {
    const input = Array.isArray(stationIds) ? stationIds : [];
    const lookup = new Map(
      (Array.isArray(activeClusterDetail?.candidates) ? activeClusterDetail.candidates : [])
        .map((candidate) => [candidate.canonical_station_id, candidate])
    );

    return input.slice().sort((a, b) => {
      const candidateA = lookup.get(a) || { canonical_station_id: a, candidate_rank: Number.MAX_SAFE_INTEGER };
      const candidateB = lookup.get(b) || { canonical_station_id: b, candidate_rank: Number.MAX_SAFE_INTEGER };
  ...
  function getDraftMergeById(mergeId) {
    return draftState.mergeItems.find((item) => item.merge_id === mergeId) || null;
  }
  ...
  function updateMergeNameAssumption() {
    ...
    const stationIds = selectedStationIds.size > 0
      ? Array.from(selectedStationIds.values())
      : getVisibleStandaloneCandidates().map((candidate) => candidate.canonical_station_id);
  ...
  function getSelectedMemberStationIds() {
    const stationIds = [];
  ...
  function getVisibleStandaloneCandidates() {
    const candidates = Array.isArray(activeClusterDetail?.candidates) ? activeClusterDetail.candidates : [];
    const hiddenMemberIds = getMergeDerivedMemberStationIds();
    if (hiddenMemberIds.size === 0) {
      return candidates.slice();
    }
    return candidates.filter((candidate) => !hiddenMemberIds.has(String(candidate?.canonical_station_id || '').trim()));
  }
  ...
  function pruneHiddenSelections() {
    ...
    if (selectedDraftMergeIds.size > 0) {
      const validMergeIds = new Set(draftState.mergeItems.map((item) => item.merge_id));
  ...
  function pruneGroupReferences() {
    ...
    for (const group of draftState.groups) {
      group.member_refs = (Array.isArray(group.member_refs) ? group.member_refs : [])
        .filter((refKey) => {
          const parsed = parseRef(refKey);
          if (parsed.type === 'candidate') {
    ...
    draftState.groups = draftState.groups.filter((group) => group.member_refs.length > 0);
  }
  ...
  function setMapMode(mode) {
    ...
    const style = nextMode === 'satellite' ? resolveSatelliteMapStyle() : resolveDefaultMapStyle();
    map.setStyle(style);
  }
  ...
  async function initMap() {
    ...
    if (typeof maplibregl === 'undefined' || !maplibregl.Map) {
      throw new Error('MapLibre did not load in the browser (check network/cache and reload).');
    }
    ...
    document.getElementById('mapModeDefaultBtn').addEventListener('click', () => setMapMode('default'));
    document.getElementById('mapModeSatelliteBtn').addEventListener('click', () => setMapMode('satellite'));
  ...
  function sortCandidatesForMarkerRings(candidates) {
    return candidates.slice().sort((left, right) => {
      ...
      return getCandidateStationId(left).localeCompare(getCandidateStationId(right));
    });
  }
  ...
  function showCandidatePopup(candidate, longitude, latitude) {
    ...
    activeMarkerPopup = new maplibregl.Popup({ closeButton: false, closeOnMove: false, offset: 10 })
  ...
  function createConcentricMarkerElement(group) {
    ...
    const ringGap = ringCount <= 1
      ? 0
      : Math.max(1.7, Math.min(4, (maxOuterRadius - baseRadius) / (ringCount - 1)));
    ...
    for (let index = 0; index < ringCount; index += 1) {
      ...
      circle.setAttribute('class', `candidate-ring ${selected ? 'selected' : ''}`);
      circle.setAttribute('pointer-events', 'stroke');
      circle.setAttribute('tabindex', '0');
      circle.setAttribute('role', 'button');
      circle.setAttribute('aria-label', `Candidate ${index + 1} of ${ringCount}: ${resolveCandidateLabel(candidate)}`);
  ...
  function buildFocusFingerprint(candidates) {
    return (candidates || [])
      ...
      .map((candidate) => `${getCandidateStationId(candidate)}:${Number(candidate.longitude).toFixed(6)}:${Number(candidate.latitude).toFixed(6)}`)
      .sort()
      .join('|');
  }
  ...
  function focusLocations(candidates) {
    if (!map) {
      pendingFocusCandidates = Array.isArray(candidates) ? candidates.slice() : [];
      document.getElementById('curationMapStatus').textContent = 'Map is still initializing. Retrying candidate rendering...';
      return;
    }
    ...
    if (!Array.isArray(candidates) || candidates.length === 0) {
      ...
      document.getElementById('curationMapStatus').textContent = 'No candidate coordinates available for this cluster.';
      return;
    }
    ...
    if (!hasValidCoordinates) {
      lastFocusFingerprint = '';
      document.getElementById('curationMapStatus').textContent = 'No valid candidate coordinates found.';
      return;
    }

    applyMarkerZoomScale();
    const overlapSummary = overlapGroupCount > 0
      ? ` ${overlapGroupCount} shared-location group(s) are shown as concentric rings.`
      : '';
    document.getElementById('curationMapStatus').textContent = `Candidate locations plotted.${overlapSummary}`;
    if (shouldFitBounds) {
  ...
  function renderClusterListMeta() {
    ...
    const scopeTag = document.getElementById('scopeTagFilter').value || 'latest';
  ...
  async function fetchClusters() {
    ...
    try {
      const res = await fetch(buildClusterListUrl());
      const payload = await res.json().catch(() => null);

      if (!res.ok) {
        const message = payload?.error || `HTTP ${res.status}`;
        if (String(message).toLowerCase().includes('unknown api endpoint')) {
          throw new Error('Cluster API not available in running server. Rebuild/restart orchestrator.');
        }
        throw new Error(message);
      }

      clusterItems = Array.isArray(payload) ? payload : [];
      renderClusterList();
      renderClusterListMeta();

      if (activeClusterSummary) {
        const refreshed = clusterItems.find((item) => item.cluster_id === activeClusterSummary.cluster_id);
        if (refreshed) {
  ...
  function renderClusterList() {
    ...
    if (!Array.isArray(clusterItems) || clusterItems.length === 0) {
      listEl.innerHTML = '<p class="muted">No clusters found for this filter.</p>';
      return;
    }

    for (const cluster of clusterItems) {
      const isActive = activeClusterSummary && activeClusterSummary.cluster_id === cluster.cluster_id;
  ...
  function selectAllCandidates() {
    selectedStationIds = new Set(getVisibleStandaloneCandidates().map((candidate) => candidate.canonical_station_id));
    selectedDraftMergeIds.clear();
    renderClusterDetail();
  }
  ...
  function renderSelectedServiceContext() {
    ...
    incomingEl.innerHTML = incomingList.length === 0
      ? '<p class="muted tiny">No incoming service context for selected nodes.</p>'
      : incomingList.map((value) => `<div class="service-item">${escapeHtml(value)}</div>`).join('');
    outgoingEl.innerHTML = outgoingList.length === 0
      ? '<p class="muted tiny">No outgoing service context for selected nodes.</p>'
      : outgoingList.map((value) => `<div class="service-item">${escapeHtml(value)}</div>`).join('');
  }
  ...
  function beginInlineRename(refKey) {
    ...
    if (parsed.type === 'candidate') {
      defaultValue = draftState.renameByRef[refKey] || String(getCandidateByStationId(parsed.id)?.display_name || '').trim();
    } else if (parsed.type === 'merge') {
      const mergeItem = getDraftMergeById(parsed.id);
      defaultValue = draftState.renameByRef[refKey] || resolveDraftMergeDisplayName(mergeItem);
    } else if (parsed.type === 'group') {
      const group = draftState.groups.find((item) => item.group_id === parsed.id);
      defaultValue = draftState.renameByRef[refKey] || String(group?.section_name || '').trim();
    }
  ...
  function createGroupFromSelection() {
    ...
    if (selectedRefs.length === 0) {
      showNotice('Select candidates or merged items before creating a group.', 'error');
      return;
    }

    const groupNameInput = String(document.getElementById('groupNameInput').value || '').trim();
    const sectionType = String(document.getElementById('groupSectionPresetType').value || 'other').trim() || 'other';

    const inferredCandidate = getCandidateByStationId(parseRef(selectedRefs[0]).id || '');
    const inferredName = groupNameInput
      || getRefTitle(selectedRefs[0])
      || (inferredCandidate ? resolveCandidateLabel(inferredCandidate) : `Group ${draftState.groups.length + 1}`);
  ...
  function removeGroup(groupId) {
    draftState.groups = draftState.groups.filter((group) => group.group_id !== groupId);
  ...
  function removeGroupMember(groupId, refKey) {
    ...
    group.member_refs = group.member_refs.filter((memberRef) => memberRef !== refKey);
  ...
  function addSelectionToGroup() {
    const groupId = String(document.getElementById('groupTargetSelect').value || '').trim();
    if (!groupId) {
    ...
    group.member_refs = uniqueStrings([...(group.member_refs || []), ...selectedRefs]);
    clearSelection();
    ensurePairWalkDefaults();
    renderClusterDetail();
  }
  ...
  function pairKey(a, b) {
    const values = [String(a || '').trim(), String(b || '').trim()].sort((x, y) => x.localeCompare(y));
  ...
  function ensurePairWalkDefaults() {
    ...
    for (let i = 0; i < groups.length; i += 1) {
      for (let j = i + 1; j < groups.length; j += 1) {
        ...
        const existing = Number.parseInt(String(draftState.pairWalkMinutesByKey[key] ?? ''), 10);
  ...
  function setPairWalkMinutes(groupA, groupB, minutes) {
    ...
    draftState.pairWalkMinutesByKey[key] = Number.isFinite(parsed) && parsed >= 0 ? parsed : 5;
    updateEditPreview();
  }

  function removeDraftMerge(mergeId) {
    draftState.mergeItems = draftState.mergeItems.filter((item) => item.merge_id !== mergeId);
    selectedDraftMergeIds.delete(mergeId);
    const mergeRef = toMergeRef(mergeId);
    delete draftState.renameByRef[mergeRef];
    for (const group of draftState.groups) {
      group.member_refs = (group.member_refs || []).filter((refKey) => refKey !== mergeRef);
    }
  ...
  function stageMergeFromSelection() {
    const selected = sortStationIdsByCandidateRank(Array.from(selectedStationIds.values()));
    ...
    const inputName = String(document.getElementById('editMergeRenameInput').value || '').trim();
  ...
  function stageSplitFromSelection() {
    const selected = sortStationIdsByCandidateRank(Array.from(selectedStationIds.values()));
    if (selected.length < 2) {
    ...
    draftState.groups = [
      ...
      {
        group_id: createDraftId('grp'),
        section_type: inferCandidateCategory(getCandidateByStationId(second[0])),
    ...
    ensurePairWalkDefaults();
    setActiveTool('group');
    showNotice('Split groups staged locally. You can refine them in Group mode.', 'success');
    renderClusterDetail();
  }
  ...
  function buildRenameTargetsPayload() {
    const targets = [];
    for (const [refKey, renameTo] of Object.entries(draftState.renameByRef || {})) {
      const parsed = parseRef(refKey);
      ...
      const originalName = String(getCandidateByStationId(parsed.id)?.display_name || '').trim();
  ...
  function buildSegmentWalkLinksFromGroups(groups, stationToSegment) {
    ...
    for (let i = 0; i < groups.length; i += 1) {
      for (let j = i + 1; j < groups.length; j += 1) {
        ...
        const segmentA = stationToSegment.get(groupA.member_station_ids[0]) || '';
        const segmentB = stationToSegment.get(groupB.member_station_ids[0]) || '';
        ...
        links.push({
          ...
          min_walk_minutes: Number.parseInt(String(draftState.pairWalkMinutesByKey[pairKey(groupA.group_id, groupB.group_id)] ?? 5), 10) || 5,
          bidirectional: true,
          metadata: {
  ...
  function buildSplitGroupsFromDraft() {
    ...
    for (const draftGroup of draftState.groups) {
      const memberIds = uniqueStrings(
        (draftGroup.member_refs || []).flatMap((refKey) => getRefMemberStationIds(refKey))
      );
      ...
      groups.push({
        ...
        section_type: String(draftGroup.section_type || 'other').trim() || 'other',
    ...
    if (groups.length < 2) {
      throw new Error('Split/group resolve needs at least two non-empty groups.');
    }

    const stationToSegment = new Map(
      (Array.isArray(activeClusterDetail?.candidates) ? activeClusterDetail.candidates : [])
        .map((candidate) => [candidate.canonical_station_id, String(candidate?.segment_context?.segment_id || '').trim()])
  ...
  function buildResolvePayload() {
    ...
    const note = String(document.getElementById('editNoteInput').value || '').trim();
    ...
    const selectedCandidates = uniqueStrings(sortStationIdsByCandidateRank(Array.from(selectedStationIds.values())));
    ...
    if (activeTool === 'split') {
      ...
      const groups = [
        {
          group_label: 'Split A',
          section_type: inferCandidateCategory(getCandidateByStationId(left[0])),
          section_name: 'Split A',
        ...
        {
          group_label: 'Split B',
          section_type: inferCandidateCategory(getCandidateByStationId(right[0])),
          section_name: 'Split B',
    ...
    const renameInput = String(document.getElementById('editMergeRenameInput').value || '').trim();
    const renameTo = renameInput || resolveMergeNameAssumption(selectedCandidates);
    return {
      operation: 'merge',
      selected_station_ids: selectedCandidates,
      groups: [
  ...
  function updateToolAvailability() {
    ...
    const groupAvailable = selectedStationIds.size > 0 || selectedDraftMergeIds.size > 0 || draftState.groups.length > 0;
  ...
  function renderPairWalkEditor() {
    ...
    if (draftState.groups.length < 2) {
      container.innerHTML = '<p class="muted tiny">Pairwise links appear when at least two groups exist.</p>';
      return;
    }
    ...
    for (let i = 0; i < draftState.groups.length; i += 1) {
      for (let j = i + 1; j < draftState.groups.length; j += 1) {
        ...
        input.addEventListener('input', (event) => {
          setPairWalkMinutes(groupA.group_id, groupB.group_id, event.target.value);
        });
  ...
  function renderGroupEditor() {
    ...
    for (const group of draftState.groups) {
      ...
      const memberRows = (group.member_refs || []).map((refKey) => {
        const parsed = parseRef(refKey);
      ...
      card.querySelectorAll('[data-remove-group]').forEach((button) => {
        button.addEventListener('click', () => removeGroup(button.getAttribute('data-remove-group')));
      });

      card.querySelectorAll('[data-remove-group-member]').forEach((button) => {
        button.addEventListener('click', () => {
          const value = String(button.getAttribute('data-remove-group-member') || '').trim();
      ...
      card.querySelectorAll('[data-group-name]').forEach((input) => {
        input.addEventListener('change', (event) => {
          updateGroupName(input.getAttribute('data-group-name'), event.target.value);
        });
      });

      card.querySelectorAll('[data-group-type]').forEach((select) => {
        select.addEventListener('change', (event) => {
          updateGroupType(select.getAttribute('data-group-type'), event.target.value);
        });
      });
  ...
  function updateEditPreview() {
    ...
    try {
      ...
      const summary = payload.operation === 'merge'
  ...
  async function resolveConflict() {
    ...
    try {
      const payload = buildResolvePayload();
      const res = await fetch(`/api/qa/v2/clusters/${encodeURIComponent(activeClusterDetail.cluster_id)}/decisions`, {
        method: 'POST',
      ...
      showNotice(`Conflict resolved (decision id=${data.decision_id || 'n/a'}).`, 'success');
      clearSelection();
      resetDraftState();
      await fetchClusters();
  ...
  function resolveCandidateNameById(canonicalStationId) {
    ...
    const candidates = Array.isArray(activeClusterDetail?.candidates) ? activeClusterDetail.candidates : [];
    const match = candidates.find((candidate) => candidate.canonical_station_id === cleanId);
    if (!match) {
      return cleanId;
    }
  ...
  function renderDraftMergesInline(candidatesEl) {
    ...
    for (const mergeItem of draftState.mergeItems) {
      ...
      const memberRows = mergeItem.member_station_ids.map((stationId) => {
        return `
          <li class="curated-member-item">
  ...
  function renderCuratedCandidatesInline(candidatesEl) {
    if (!candidatesEl || !Array.isArray(activeCuratedProjectionItems) || activeCuratedProjectionItems.length === 0) {
      return;
    }
    ...
    for (const item of activeCuratedProjectionItems) {
      ...
      const fieldNameSource = (Array.isArray(item.field_provenance) ? item.field_provenance : [])
      ...
      const memberRows = members.map((member) => {
        ...
        const role = String(member?.member_role || 'member').trim() || 'member';
        return `
          <li class="curated-member-item">
  ...
  function renderClusterDetail() {
    ...
    headerEl.textContent = activeClusterDetail.display_name || activeClusterDetail.cluster_id;
    ...
    if (visibleCandidates.length === 0) {
      ...
      emptyStandaloneMessage.textContent = 'No standalone candidates remain. Members are shown inside merged derived cards.';
    ...
    for (const candidate of visibleCandidates) {
      ...
      const providers = Array.isArray(candidate.provider_labels) ? candidate.provider_labels.filter(Boolean) : [];
      const lines = Array.isArray(candidate?.service_context?.lines) ? candidate.service_context.lines.slice(0, 8) : [];
    ...
    if (historyRows.length === 0) {
      decisionsEl.innerHTML = '<p class="muted">No applied edits recorded for this cluster.</p>';
    } else {
      historyRows
  ...
  async function loadClusterDetail(clusterId, resetSelection = true) {
    try {
      ...
      const [res, curatedRes] = await Promise.all([
        fetch(`/api/qa/v2/clusters/${encodeURIComponent(clusterId)}`),
      ...
      activeCuratedProjectionItems = Array.isArray(curatedRes) ? curatedRes : [];
      activeClusterSummary = clusterItems.find((item) => item.cluster_id === clusterId) || null;
  ...
  function bindEvents() {
    document.getElementById('refreshBtn').addEventListener('click', fetchClusters);
    document.getElementById('countryFilter').addEventListener('change', fetchClusters);
    document.getElementById('statusFilter').addEventListener('change', fetchClusters);
    document.getElementById('scopeTagFilter').addEventListener('change', fetchClusters);

    document.getElementById('candidateSelectAllBtn').addEventListener('click', selectAllCandidates);
    document.getElementById('candidateClearBtn').addEventListener('click', clearCandidateSelection);
    ...
    const createGroupBtn = document.getElementById('createGroupFromSelectionBtn');
    if (createGroupBtn) {
    ...
    document.getElementById('resolveConflictBtn').addEventListener('click', resolveConflict);

    document.getElementById('editMergeRenameInput').addEventListener('input', updateEditPreview);
    document.getElementById('groupNameInput').addEventListener('input', updateEditPreview);
    document.getElementById('groupSectionPresetType').addEventListener('change', updateEditPreview);
    document.getElementById('groupTargetSelect').addEventListener('change', updateEditPreview);

    document.getElementById('editNoteInput').addEventListener('input', (event) => {
