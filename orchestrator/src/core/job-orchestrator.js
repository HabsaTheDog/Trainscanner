...
function computeBackoffMs(attempt, options = {}) {
  ...
  const jitterPct = Number.isFinite(options.jitterPct) ? options.jitterPct : 0.15;
...
function isDuplicateIdempotencyError(err) {
  ...
  const appErr = toAppError(err);
  const message = `${String(appErr.message || '')} ${String(err.message || '')}`.toLowerCase();
...
function isRunningSlotConflictError(err) {
  ...
  return (
    ...
    (
      ...
      (message.includes('pipeline_jobs') && message.includes('status') && message.includes('running'))
    )
  );
}
...
function createJobOrchestrator(options = {}) {
  ...
  const transientCodes = new Set(
    Array.isArray(options.transientErrorCodes)
      ...
      : ['MOTIS_UNAVAILABLE', 'INTERNAL_ERROR', 'SOURCE_FETCH_FAILED', 'SOURCE_VERIFY_FAILED', 'INGEST_FAILED']
  );

  async function runJob(input = {}) {
    const jobType = String(input.jobType || '').trim();
    const idempotencyKey = String(input.idempotencyKey || '').trim();
    const runContext = input.runContext && typeof input.runContext === 'object' ? input.runContext : {};
    const maxAttempts = Number.isFinite(input.maxAttempts) ? Math.max(1, input.maxAttempts) : 3;
    ...
    while (attempt < maxAttempts) {
      ...
      if (!runningJob) {
        ...
        const currentRunning = await jobsRepo.countRunningByType(jobType).catch(() => maxConcurrent);
        throw new AppError({
          code: 'JOB_BACKPRESSURE',
      ...
      try {
        ...
        const result = await execute({
          ...
          async updateCheckpoint(nextCheckpoint) {
            checkpoint = nextCheckpoint && typeof nextCheckpoint === 'object' ? nextCheckpoint : checkpoint;
