...
const SOURCE_SCHEMA = {
  ...
  properties: {
    ...
    accessType: { type: 'string', enum: ['public', 'api_key', 'token', 'other'] },
    authSetupUrl: { type: 'string' },
    licenseName: { type: 'string' },
    licenseUrl: { type: 'string' },
    attributionText: { type: 'string' },
    updateCadence: { type: 'string' },
    downloadMethod: { type: 'string', enum: ['manual_redirect', 'direct', 'api'] },
...
function validateSourceDiscoveryConfig(raw) {
  ...
  for (const source of raw.sources) {
    ...
    if (source.format === 'gtfs' && !String(source.fallbackReason || '').trim()) {
      throw new AppError({
        code: 'INVALID_CONFIG',
