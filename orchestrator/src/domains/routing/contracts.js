...
function validateRouteRequestBody(body) {
  ...
  const datetime = body && body.datetime;
  ...
  if (!isValidDate(requestDate)) {
    throw new AppError({
      code: 'INVALID_REQUEST',
      statusCode: 400,
      message: 'Invalid datetime. Use ISO-8601 format, e.g. 2026-02-19T12:00:00Z.'
    });
  }
  ...
  if (requestDate < min || requestDate > max) {
    throw new AppError({
      code: 'INVALID_REQUEST',
      statusCode: 400,
      message: 'Datetime outside supported range. Pick a time between now - 30 days and the next 400 days.',
      details: {
