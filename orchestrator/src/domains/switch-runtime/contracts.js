...
function validateRuntimeDescriptor(runtime, contextPath) {
  ...
  const mode = String(runtime.mode || runtime.source || 'canonical-export').trim();
  ...
  if (runtime.asOf && runtime.asOf !== 'latest' && !ISO_DATE_RE.test(runtime.asOf)) {
    throw new AppError({
      code: 'INVALID_CONFIG',
...
function validateGtfsProfilesConfig(raw) {
  ...
  if (!source || typeof source !== 'object' || Array.isArray(source)) {
    throw new AppError({
      code: 'INVALID_CONFIG',
      message: 'GTFS profiles config must be an object or { profiles: { ... } }'
    });
  }
