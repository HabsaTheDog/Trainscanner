...
(shouldRun ? test : test.skip)('qa v2 cluster generation is deterministic and decisions are atomic', async () => {
  ...
  execFileSync('bash', [path.join(repoRoot, 'scripts', 'data', 'db-migrate.sh'), '--quiet'], {
    cwd: repoRoot,
    stdio: 'inherit',
    env: {
      ...process.env,
  ...
  const firstRows = await client.queryRows(`SELECT cluster_id FROM qa_station_clusters_v2 WHERE country = 'DE' AND scope_tag = 'latest' ORDER BY cluster_id`);

  await client.exec(`SELECT qa_rebuild_station_clusters_v2('DE', NULL);`);
  const secondRows = await client.queryRows(`SELECT cluster_id FROM qa_station_clusters_v2 WHERE country = 'DE' AND scope_tag = 'latest' ORDER BY cluster_id`);

  assert.deepEqual(firstRows.map((row) => row.cluster_id), secondRows.map((row) => row.cluster_id));

  const listUrl = new URL('http://localhost/api/qa/v2/clusters?country=DE&scope_tag=latest&limit=20');
  const clusters = await getReviewClustersV2(listUrl);
  const cluster = clusters.find((row) =>
    Array.isArray(row.candidates) && row.candidates.some((candidate) => candidate.canonical_station_id === stationA)
  );
  assert.ok(cluster, 'expected seeded cluster in v2 list response');
  assert.equal(Object.prototype.hasOwnProperty.call(cluster, 'queue_items'), false);

  const detail = await getReviewClusterDetailV2(cluster.cluster_id);
  assert.ok(Array.isArray(detail.candidates));
  assert.ok(detail.candidates.some((candidate) => candidate.canonical_station_id === stationA));
  assert.equal(Object.prototype.hasOwnProperty.call(detail, 'queue_items'), false);
  const candidateWithService = detail.candidates.find((candidate) => candidate.canonical_station_id === stationA);
  ...
  assert.ok(mergeCuratedMembers.some((row) => row.canonical_station_id === stationA));
  assert.ok(mergeCuratedMembers.some((row) => row.canonical_station_id === stationB));

  const curatedListUrl = new URL(`http://localhost/api/qa/v2/curated-stations?cluster_id=${encodeURIComponent(cluster.cluster_id)}&status=active&limit=20`);
  ...
  const curatedDetail = await getCuratedStationDetailV1(curatedList[0].curated_station_id);
  assert.equal(curatedDetail.curated_station_id, curatedList[0].curated_station_id);
  ...
  assert.ok(splitCuratedRows.some((row) => row.status === 'active' && row.derived_operation === 'split' && row.items >= 2));
  assert.ok(splitCuratedRows.some((row) => row.status === 'superseded' && row.derived_operation === 'merge'));

  const splitCuratedListUrl = new URL(`http://localhost/api/qa/v2/curated-stations?cluster_id=${encodeURIComponent(cluster.cluster_id)}&status=active&limit=20`);
  const splitCuratedList = await getCuratedStationsV1(splitCuratedListUrl);
  assert.ok(splitCuratedList.length >= 2);
  assert.ok(splitCuratedList.every((row) => row.derived_operation === 'split'));
  ...
  assert.ok(sectionLinks.every((row) => Number.parseInt(String(row.min_walk_minutes || 0), 10) >= 0));
});
