...
test('e2e profile activation and route smoke/regression', async (t) => {
  ...
  await fs.writeFile(path.join(frontendDir, 'index.html'), '<html><body>ok</body></html>\n', 'utf8');
  ...
  const motisServer = await startHttpServer((req, res) => {
    ...
    if (url.pathname === '/api/v5/plan') {
      const from = url.searchParams.get('fromPlace') || url.searchParams.get('from') || '';
      const to = url.searchParams.get('toPlace') || url.searchParams.get('to') || '';
      if (!from || !to) {
  ...
  await waitFor(async () => {
    ...
    if (status.body.state === 'failed') {
      ...
      throw new Error(`switch failed: ${JSON.stringify(status.body)}\n${logs.stderr}\n${logs.stdout}`);
    }
    return null;
  }, { timeoutMs: 15000, intervalMs: 200 });
  ...
  assert.equal(taggedRoute.body.routeRequestResolved.destination.strategy, 'tagged_stop_id');
  ...
  assert.equal(lookupRoute.body.routeRequestResolved.origin.strategy, 'station_lookup');
  assert.equal(lookupRoute.body.routeRequestResolved.destination.strategy, 'station_lookup');

  await execFileAsync(process.execPath, [
    path.join(repoRoot, 'orchestrator', 'src', 'cli', 'run-route-regression.js'),
    '--api-url',
    apiUrl,
    '--cases',
