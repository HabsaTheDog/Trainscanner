...
const { serveMvtTile, validateTileCoords } = require("../../src/domains/qa/mvt");
...
test("serveMvtTile SQL contains ST_AsMVT for canonical layer", async () => {
    ...
    assert.ok(firstCall.sql.includes("ST_AsMVT"), "first query should use ST_AsMVT");
    assert.ok(firstCall.sql.includes("'canonical'"), "first query should specify canonical layer");
});
...
test("serveMvtTile SQL contains ST_TileEnvelope with correct z/x/y", async () => {
    ...
    for (const call of client.calls) {
        assert.ok(call.sql.includes("ST_TileEnvelope"), "should use ST_TileEnvelope");
...
test("serveMvtTile SQL contains ST_AsMVT for staging layer", async () => {
    ...
    assert.ok(secondCall.sql.includes("ST_AsMVT"), "second query should use ST_AsMVT");
    assert.ok(secondCall.sql.includes("'staging'"), "second query should specify staging layer");
});
...
test("serveMvtTile concatenates tile buffers from both layers", async () => {
    ...
    const client = makeMockClient({ canonicalTileHex: canonicalHex, stagingTileHex: stagingHex });
    const buf = await serveMvtTile(client, { z: 10, x: 549, y: 335 });
    const expected = Buffer.concat([
        Buffer.from(canonicalHex, "hex"),
        Buffer.from(stagingHex, "hex"),
    ]);
    assert.deepEqual(buf, expected, "should concatenate canonical and staging tile bytes");
});
...
test("serveMvtTile throws on invalid tile coordinates", async () => {
    ...
    assert.equal(client.calls.length, 0, "should not call DB if coords are invalid");
});
