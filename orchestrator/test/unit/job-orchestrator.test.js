...
test('computeBackoffMs returns bounded exponential values', () => {
  for (let i = 1; i <= 10; i += 1) {
    const value = computeBackoffMs(i, { baseMs: 100, maxMs: 500, jitterPct: 0 });
    assert.ok(value >= 100);
    assert.ok(value <= 500);
  }
});
...
test('isRunningSlotConflictError detects running-slot unique constraint conflicts', () => {
  assert.equal(
    isRunningSlotConflictError(new Error('duplicate key value violates unique constraint "idx_pipeline_jobs_one_running_per_type"')),
    true
  );

  assert.equal(
    isRunningSlotConflictError(new Error('duplicate key value violates unique constraint "pipeline_jobs_job_type_idempotency_key_key"')),
    false
  );
});
...
test('job orchestrator resumes queued job for the same idempotency key', async () => {
  ...
  const orchestrator = createJobOrchestrator({ jobsRepo, sleep: async () => {} });
  let executeCalls = 0;
  const result = await orchestrator.runJob({
    jobType: 'canonical.build-stations',
    idempotencyKey: 'queued-key',
...
test('job orchestrator maps running-slot claim races to JOB_BACKPRESSURE', async () => {
  ...
  jobsRepo.markRunning = async () => {
    throw new Error('duplicate key value violates unique constraint "idx_pipeline_jobs_one_running_per_type"');
  };
