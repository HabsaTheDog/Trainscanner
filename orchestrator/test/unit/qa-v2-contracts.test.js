...
test('normalizeClusterDecision validates merge payload and keeps line decision scaffold', () => {
  ...
  assert.deepEqual(payload.lineDecisions, { future_line_groups: [{ id: 'l1' }] });
});
...
test('resolveCandidateDisplayName prefers explicit display name and never falls back to raw id', () => {
  assert.equal(resolveCandidateDisplayName({ display_name: 'Berlin Hbf', canonical_name: 'Berlin Hauptbahnhof' }), 'Berlin Hbf');
  assert.equal(resolveCandidateDisplayName({ canonical_name: 'Munchen Hbf', canonical_station_id: 'cstn_hash' }), 'Munchen Hbf');
  assert.equal(resolveCandidateDisplayName({ canonical_station_id: 'cstn_hash' }), 'Unnamed station');
});
